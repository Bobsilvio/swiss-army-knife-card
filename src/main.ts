...
import SAK_BUILTIN_TEMPLATES from './sak-builtin-templates';
import './sak-layout-card';
...
class SwissArmyKnifeCard extends LitElement {
  ...
  constructor() {
    ...
    // Browser feature detection (from BrowserDetection module)
    this.browserDetection = new BrowserDetection();
    this.browserCapabilities = this.browserDetection.getCapabilities();
    ...
    if (!this.lovelace) {
      // Lovelace not yet available during construction (timing edge case).
      // setConfig will initialize it before any card functionality is used.
      console.warn("card::constructor - Lovelace not available yet, will initialize in setConfig");
    }
  ...
  */
  ...
  static getUserStyles() {
    ...
    if ((SwissArmyKnifeCard._sakUserTemplates)
        && (SwissArmyKnifeCard._sakUserTemplates.definitions?.user_css_definitions)) {
      this.userContent = SwissArmyKnifeCard._sakUserTemplates.definitions.user_css_definitions.reduce((accumulator, currentValue) => accumulator + currentValue.content, '');
    }
  ...
  static getSakStyles() {
    ...
    if ((SwissArmyKnifeCard._sakSysTemplates)
        && (SwissArmyKnifeCard._sakSysTemplates.definitions?.sak_css_definitions)) {
      this.sakContent = SwissArmyKnifeCard._sakSysTemplates.definitions.sak_css_definitions.reduce((accumulator, currentValue) => accumulator + currentValue.content, '');
    }
  ...
  static getSakSvgDefinitions() {
    ...
    if ((SwissArmyKnifeCard._sakSysTemplates)
        && (SwissArmyKnifeCard._sakSysTemplates.definitions?.sak_svg_definitions)) {
      sakSvgContent = SwissArmyKnifeCard._sakSysTemplates.definitions.sak_svg_definitions.reduce((accumulator, currentValue) => accumulator + currentValue.content, '');
    }
  ...
  static getUserSvgDefinitions() {
    ...
    if ((SwissArmyKnifeCard._sakUserTemplates)
        && (SwissArmyKnifeCard._sakUserTemplates.definitions?.user_svg_definitions)) {
      userSvgContent = SwissArmyKnifeCard._sakUserTemplates.definitions.user_svg_definitions.reduce((accumulator, currentValue) => accumulator + currentValue.content, '');
    }
  ...
  */
  static get styles() {
    ...
    if (!SwissArmyKnifeCard.lovelace) {
      // Lovelace not ready yet (timing issue during element registration).
      // Return minimal styles now; full SAK/user styles will be injected in setConfig.
      console.warn("SAK - Can't get reference to Lovelace yet, deferring SAK styles to setConfig");
      return css`${SwissArmyKnifeCard.getSystemStyles()}`;
    }
    // Resolve sys templates: prefer lovelace.config (user-defined), fall back to built-in.
    // Never mutate lovelace.config — it may be frozen in newer HA versions.
    if (!SwissArmyKnifeCard._sakSysTemplates) {
      SwissArmyKnifeCard._sakSysTemplates =
        SwissArmyKnifeCard.lovelace.config.sak_sys_templates ?? SAK_BUILTIN_TEMPLATES;
    }
    // Resolve user templates: read from lovelace.config (may be null — that is fine).
    if (!SwissArmyKnifeCard._sakUserTemplates) {
      SwissArmyKnifeCard._sakUserTemplates =
        SwissArmyKnifeCard.lovelace.config.sak_user_templates ?? null;
    }
    ...
    SwissArmyKnifeCard._sakStylesFromLovelace = true;
    ...
    for (value of this.config.entities) {
      ...
      if (this.config.entities[index].attribute) {
        ...
        if (arrayPos !== -1) {
          ...
          attrMore = this.config.entities[index].attribute.substring(arrayPos, this.config.entities[index].attribute.length);
          ...
          arrayMap = attrMore.substring(bracketEnd + 2, attrMore.length);

          // Fetch state with null-safety checks
          const attrArray = this.entities[index].attributes?.[attribute];
          if (attrArray && attrArray[arrayIdx] !== undefined) {
            attributeState = arrayMap ? attrArray[arrayIdx][arrayMap] : attrArray[arrayIdx];
          } else {
          ...
          attrMore = this.config.entities[index].attribute.substring(dotPos, this.config.entities[index].attribute.length);
          arrayMap = attrMore.substring(1, attrMore.length);

          // Fetch state with null-safety check
          const attrObj = this.entities[index].attributes?.[attribute];
          attributeState = attrObj !== undefined ? attrObj[arrayMap] : undefined;
  ...
  */
  ...
  setConfig(config) {
    ...
    // Deferred Lovelace initialization: if Lovelace was not available during construction
    // (timing edge case), get it now. By the time setConfig is called, Lovelace is always ready.
    if (!this.lovelace) {
      SwissArmyKnifeCard.lovelace = Utils.getLovelace();
      this.lovelace = SwissArmyKnifeCard.lovelace;
    }
    if (!this.lovelace) {
      throw Error("card::setConfig - Can't get Lovelace panel");
    }
    // Resolve sys templates (never mutate lovelace.config — it may be frozen in newer HA).
    if (!SwissArmyKnifeCard._sakSysTemplates) {
      if (!this.lovelace.config.sak_sys_templates) {
        console.info(`${version} - SAK - System Templates not found in Lovelace config. Using built-in templates.`);
      }
      SwissArmyKnifeCard._sakSysTemplates =
        this.lovelace.config.sak_sys_templates ?? SAK_BUILTIN_TEMPLATES;
    }

    // Resolve user templates: merge lovelace.config + window.__sakUserTemplates (from sak-layout).
    // Build a new merged object — never mutate lovelace.config.
    const globalSakTemplates = (window as any).__sakUserTemplates;
    const lovelaceUserTemplates = this.lovelace.config.sak_user_templates;
    if (globalSakTemplates?.templates && Object.keys(globalSakTemplates.templates).length > 0) {
      SwissArmyKnifeCard._sakUserTemplates = {
        ...(lovelaceUserTemplates ?? {}),
        templates: {
          ...(globalSakTemplates.templates ?? {}),
          ...(lovelaceUserTemplates?.templates ?? {}),
        },
      };
    } else if (lovelaceUserTemplates && !SwissArmyKnifeCard._sakUserTemplates) {
      SwissArmyKnifeCard._sakUserTemplates = lovelaceUserTemplates;
    }

    // If static get styles ran without Lovelace (deferred case), load SVG defs once
    // and inject SAK/user CSS dynamically into each instance's shadow root.
    if (!SwissArmyKnifeCard._sakStylesFromLovelace) {
      // Load SVG definitions once (class-level, not per instance)
      if (!SwissArmyKnifeCard._sakDefsLoaded) {
        SwissArmyKnifeCard.getSakSvgDefinitions();
        SwissArmyKnifeCard.getUserSvgDefinitions();
        SwissArmyKnifeCard._sakDefsLoaded = true;
      }
      // Inject SAK/user CSS into this instance's shadow root (per instance)
      if (this.shadowRoot) {
        const sakCss = SwissArmyKnifeCard.getSakStyles().cssText || '';
        const userCss = SwissArmyKnifeCard.getUserStyles().cssText || '';
        if (sakCss || userCss) {
          const style = document.createElement('style');
          style.textContent = sakCss + userCss;
    ...
    if (!validationResult.isValid) {
      validationResult.errors.forEach((err) => {
        console.error(`[SAK] Config error at '${err.path}': ${err.message}`);
      });
    }
    if (validationResult.warnings.length > 0) {
      validationResult.warnings.forEach((warn) => {
        console.warn(`[SAK] Config warning at '${warn.path}': ${warn.message}`);
      });
    }
    ...
    function findTemplate(key, value) {
      ...
      if (value?.template) {
        const userTemplates = SwissArmyKnifeCard._sakUserTemplates;
        const template = userTemplates?.templates?.[value.template.name];
        if (!template) {
    ...
    if (this.dev.m3) {
      ...
      if (SwissArmyKnifeCard._sakUserTemplates?.templates?.m3) {
        const { m3 } = SwissArmyKnifeCard._sakUserTemplates.templates;
        ...
        function hex2rgb(hexColor) {
          const rgbCol = {} as Record<string, any>;
        ...
        function getSurfaces(surfaceColor, paletteColor, opacities, cssName, mode) {
          const bgCol = {} as Record<string, any>;
          const fgCol = {} as Record<string, any>;
        ...
        for (const [index, cssName] of Object.entries(cssNames)) { // lgtm[js/unused-local-variable]
          if ((cssName as any).substring(0, 9) === 'theme-ref') {
            themeDefs += `  ${cssName}\n`;
...
// https://github.com/home-assistant/frontend/blob/8580d3f9bf59ffbcbe4187a0d7a58cc23d9822df/src/dialogs/more-info/components/lights/ha-more-info-light-brightness.ts#L76
  ...
  async fetchRecent(entityId, start, end, skipInitialState) {
    ...
    try {
      ...
      if (this.dev.debug) console.warn('[SAK] fetchRecent failed for', entityId, err);
      return [];
    }
  }
