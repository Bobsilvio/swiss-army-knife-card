...
export default class Utils {
  ...
  */
  ...
  static getLovelace() {
    ...
    root = root && root.shadowRoot;
    // Try both old (app-drawer-layout) and new (ha-drawer) HA DOM structures
    let resolver = root && root.querySelector('app-drawer-layout partial-panel-resolver, ha-drawer partial-panel-resolver');
    // Fallback: partial-panel-resolver directly in shadow root (newer HA versions)
    if (!resolver) resolver = root && root.querySelector('partial-panel-resolver');
    root = resolver;
    ...
    if (root) {
      ...
      if (ll) ll.current_view = root.___curView;
      return ll || null;
    }
    return null;
  }
