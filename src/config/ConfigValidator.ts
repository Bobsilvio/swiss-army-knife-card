...
// Known tool types — must match the keys in toolset.ts toolsNew map
const KNOWN_TOOL_TYPES = new Set([
  'area', 'badge', 'bar', 'circle', 'circslider', 'ellipse',
  'horseshoe', 'icon', 'line', 'name', 'progpath', 'rectangle',
  'rectex', 'regpoly', 'segarc', 'slider', 'sparkline',
  'state', 'switch', 'text', 'usersvg',
]);
...
export class ConfigValidator {
  ...
  private validateLayout(config: SakConfig): void {
    ...
    // Toolsets may be provided later via templates — skip validation here
  ...
  private validatePosition(position: any, path: string): void {
    ...
    // cx and cy can be any number — positions outside 0-100 are valid in SAK
    if (position.cx !== undefined && typeof position.cx !== 'number') {
      this.addWarning(`${path}.cx`, 'cx should be a number', 'INVALID_CX');
    }

    if (position.cy !== undefined && typeof position.cy !== 'number') {
      this.addWarning(`${path}.cy`, 'cy should be a number', 'INVALID_CY');
    }
  }
